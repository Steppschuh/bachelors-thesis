...
I presume a basic understanding of how the Android activity\cite{androiddocs:activity} lifecycle\improvement{Explain} works.
...
The |DataRequestResponse|\cite{sensordatalogger:datarequestresponse} class for example holds a list of |DataBatches| and is the responsible for exchaning sensor data with the connected mobile device.
...
The JSON string can be converted to a byte array, which can be transferred as described in section \ref{sec:transferring}.
It is crucial that the same character encoding is used, we stick with UTF-8.

\subsection{Transferring Data}\label{sec:transferring}
...
\end{itemize}
Payloads are not required by default because messages are a one-way communication mechanism, often used to only trigger RPCs\footnote{Remote procedure calls}. We use this to pass the serialized sensor data.
...
We have described how we can access, persist and transfer sensor data.
